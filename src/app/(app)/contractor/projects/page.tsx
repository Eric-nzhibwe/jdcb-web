...
export default function ContractorProjectsPage() {
  ...
  useEffect(() => {
    if (!user) return;
    const unsub = subscribeToProjectsByContractor(user.id, (p) => { setProjects(p); setLoading(false); });
    return unsub;
  }, [user?.id]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => router.push('/contractor/projects/new')}>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {PROJECT_STATUSES.map((s) => {
          ...
          return (
            <button key={s.value} onClick={() => setFilter(s.value as Filter)}
              className={`px-4 py-1.5 rounded-full text-xs font-semibold border transition-colors ${filter === s.value ? 'text-white border-transparent' : 'border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800'}`}
